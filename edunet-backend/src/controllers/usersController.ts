...
export async function createUser(req: Request, res: Response) {
  ...
  const {
    ...
    curso_asignado,
    // Support both frontend property names and DB column names
  ...
  const final_estudiante_acargo = nombre_estudiante_acargo || estudiante_relacionado;
  const final_parentezco = parentezco || parentesco;
  const final_cargo_admin = cargo_admin || cargo;
  const final_idRol = idRol || rol;
  ...
  try {
    ...
    try {
      ...
      if (Number(final_idRol) === 3 && (final_estudiante_acargo || documentoEstudiante)) {
...
export async function getUserById(req: Request, res: Response) {
  ...
  try {
    const conn = await connectDB();
    try {
      const [rows] = await conn.execute('SELECT * FROM usuarios WHERE idUsuarios = ?', [id]);
      if (Array.isArray(rows) && rows.length > 0) {
        res.json(rows[0]);
      } else {
        res.status(404).json({ error: 'Usuario no encontrado' });
    ...
    res.status(500).json({ error: 'Error al buscar usuario por ID', details: err });
  }
}
...
export async function updateUser(req: Request, res: Response) {
  ...
  try {
      ...
      try {
          ...
          // Map frontend names to backend DB names if they exist
          if (fields.estudiante_relacionado !== undefined) fields.nombre_estudiante_acargo = fields.estudiante_relacionado;
          if (fields.parentesco !== undefined) fields.parentezco = fields.parentesco;
          if (fields.cargo !== undefined) fields.cargo_admin = fields.cargo;
          if (fields.rol !== undefined) fields.idRol = fields.rol;
...
export async function deleteUserById(req: Request, res: Response) {
  ...
  try {
    const conn = await connectDB();
    try {
        const [users]: any = await conn.execute('SELECT * FROM usuarios WHERE idUsuarios = ?', [id]);
        if (users.length === 0) {
            return res.status(404).json({ error: 'Usuario a inactivar no encontrado' });
        }
        const userToInactivate = users[0];
        ...
        if (loggedInUser.id === userToInactivate.idUsuarios) {
            return res.status(403).json({ error: 'No puedes inactivarte a ti mismo' });
        }
